...
class Service:
    ...
    def filter_objects(self):
        ...
        lst=[s1,1,'2',assig4,s4,grade02,s7]
        
        new_list=self.m.filter(lst,self.filterCriteria)
        return new_list
    def comparisonFunction(self,x,y):
        if x>=y:
            return True
        return False
    ...
    def best_students(self):# returns a list of students sorted by their best average grade received for all assignments
        ...
        sorted_grouped_list=self.m.gnomeSort(grouped_list,self.comparisonFunction) #################################### here is where the gnome sort is used
        ranking=[]
